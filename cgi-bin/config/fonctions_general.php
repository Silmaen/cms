...
function GestionMenu($connexion)
{
	...
	$sql_menu="SELECT * FROM admin_menu ORDER BY ordre ASC";
...
{
	$sql="SELECT id_utilisateur_groupe, id_admin_menu, droit FROM admin_menus_groupes WHERE (id_utilisateur_groupe='".$_SESSION["id_utilisateur_groupe"]."' AND id_admin_menu='".$_SESSION["id_admin_menu_selectionne"]."')";
...
function GestionPagination($connexion, $id_table, $nom_table){
	...
	$nombre_de_pages = 0;
	$nombre_resultats = 0;
	$sql_pagination="SELECT COUNT(".$id_table.") AS nombre_resultats FROM ".$nom_table;
...
{
	$sql_duplication = "INSERT INTO admin_utilisateurs_session (id_utilisateur, id_admin_menu, colonne, colonne_titre_fr, largeur, ordre, sens_tri, tri_utilisateur) SELECT ".$id_utilisateur_nouveau.", id_admin_menu, colonne, colonne_titre_fr, largeur, ordre, sens_tri, tri_utilisateur FROM admin_utilisateurs_session WHERE id_utilisateur = '".$id_utilisateur_source."'";
...
function GestionTri($connexion, $colonne, $sens_tri, $colonne_default, $items_par_page)
{
	...
	$sql_session="SELECT id_utilisateur, id_admin_menu, colonne, sens_tri, tri_utilisateur FROM admin_utilisateurs_session WHERE (id_admin_menu='".$_SESSION["id_admin_menu_selectionne"]."' AND id_utilisateur='".$_SESSION["id_utilisateur"]."' AND tri_utilisateur='1') LIMIT 1";
	...
	$sql_session="SELECT id_utilisateur, id_admin_menu, colonne, sens_tri, items_par_page, tri_utilisateur FROM admin_utilisateurs_session WHERE (id_admin_menu='".$_SESSION["id_admin_menu_selectionne"]."' AND id_utilisateur='".$_SESSION["id_utilisateur"]."' AND colonne='".$colonne."'  AND tri_utilisateur='1') ORDER BY ordre ASC LIMIT 1";
