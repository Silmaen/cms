...
else
{
	...
	foreach ($sql_clients->fetchAll() as $row_clients) 
	{
		...
		$sql_dons=$connexion->prepare("SELECT t1.id_client, t1.id_reservation, SUM(t1.don) as somme_dons, t1.id_etat, t1.date_don 
		FROM reservations AS t1 
		WHERE (t1.id_client=:id_client AND t1.don>0 AND (t1.id_etat=1 OR t1.id_etat=2) AND t1.date_don>=:date_debut_reglements_synthese AND t1.date_don<=:date_fin_reglements_synthese)");
