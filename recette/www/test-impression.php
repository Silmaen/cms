...
$pdf->SetTitle('FICHE SYNTHESE DES REGLEMENTS PERCUS');
$pdf->SetSubject('FICHE SYNTHESE DES REGLEMENTS PERCUS');
$pdf->SetKeywords('FICHE SYNTHESE DES REGLEMENTS PERCUS');
...
else
{
	...
	foreach ($sql_clients->fetchAll() as $row_clients) 
	{
		...
		if((fmod($compteur_client, $nb_clients_par_page)==0) AND ( $row_clients['premier_affichage']==1 OR ($row_clients['premier_affichage']==0) ) )
		{
			...
			$pdf->MultiCell(200, 5, utf8_encode("<b>Liste des adhérents ayant fait au moins une réservation sans avoir payé de don pour l'année fiscale de référence : ".$annee_fiscale_reference_dons."-".($annee_fiscale_reference_dons+1)."</b>"), 0, 'C', 0, 1, 5, ($offset_titre_clients), true,'',true);
			
			$pdf->SetFont('Helvetica', '', 10);
			$pdf->MultiCell(10, 7, utf8_encode("<b>Nb 9-</b>"), 0, 'L', 0, 1, 5, ($offset_titre_colonnes), true,'',true);
