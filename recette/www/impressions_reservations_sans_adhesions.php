...
$pdf->SetTitle('LISTE DES ADHERENTS AVEC RESA SANS ADHESION');
$pdf->SetSubject('LISTE DES ADHERENTS AVEC RESA SANS ADHESION');
$pdf->SetKeywords('LISTE DES ADHERENTS AVEC RESA SANS ADHESION');
...
$annee_fiscale_reference_adhesion = $_POST["annee_fiscale_reference_adhesion"];

$sql_clients=$connexion->prepare("SELECT t1.id_client, t1.id_reservation, t1.annee_reservation, COUNT(t1.id_reservation) as nombre_reservations,
t2.association, t2.nom, t2.prenom FROM reservations AS t1 LEFT JOIN clients AS t2 ON t2.id_client=t1.id_client WHERE t2.id_statut_client<>2 AND t1.annee_reservation=:annee_fiscale_reference AND t1.date_retour<='".date("Y-m-d")."' AND t1.id_etat<>3 AND t1.id_client NOT IN (SELECT id_client FROM clients_adhesions WHERE annee=:annee_fiscale_reference AND montant) GROUP BY t1.id_client ORDER BY t2.association ASC, t2.nom ASC, t2.prenom ASC");

$sql_exec=$sql_clients->execute([":annee_fiscale_reference"=>$annee_fiscale_reference_adhesion]);	
...
else
{
	...
	foreach ($sql_clients->fetchAll() as $row_clients) 
	{
		...
		if((fmod($compteur_client, $nb_clients_par_page)==0) AND ( $row_clients['premier_affichage']==1 OR ($row_clients['premier_affichage']==0) ) )
		{
			...
			$pdf->MultiCell(200, 5, utf8_encode("<b>Liste des adhérents ayant fait au moins une réservation sans avoir payé d'adhésion pour l'année fiscale de référence : ".$annee_fiscale_reference_adhesion."-".($annee_fiscale_reference_adhesion+1)."</b>"), 0, 'C', 0, 1, 5, ($offset_titre_clients), true,'',true);
			...
			$pdf->MultiCell(80, 7, utf8_encode("<b>Adhérent</b>"), 0, 'L', 0, 1, 95, ($offset_titre_colonnes), true,'',true);			
			$pdf->MultiCell(40, 7, utf8_encode("<b>Nb de réservations</b>"), 0, 'L', 0, 1, 175, ($offset_titre_colonnes), true,'',true);			
...
$pdf->Output('fiche-reservations-sans-adhesions-'.$annee_fiscale_reference_adhesion.'-'.($annee_fiscale_reference_adhesion+1).'.pdf', 'D');
