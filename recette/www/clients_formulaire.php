...
elseif (isset($action_selectionne) AND $action_selectionne=="copier") 
{
	...
	$sql_exec=$sql->execute([":id_client"=>$id_client_selectionne, ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
	if(!$sql_exec) echo "COPIER VALIDER - DONNEES : Pb d'accès à la table clients";
	else
	{
...
{	
	...
	if(!$sql_exec) echo "MODIFIER - DONNEES : Pb d'accès à la table clients";
	else
	{
		foreach ($sql->fetchAll() as $row) 
		{
			...
			for ($i=$nombre_lignes_a_creer; $i>=1; $i--)
			{
				...
				//print "CCC annee_debut=".($annee_premiere_adhesion+$i)."annee_fin=".($annee_premiere_adhesion+$i+1)."<br />";

				// VERSION APRES CHANGEMENT DE l'ANNEE FISCALE PRISE EN COMPTE POUR LES DONS - DEBUT 2023-12-27 
				$sql_dons=$connexion->prepare("SELECT SUM(t1.don) as total_don, t1.id_client, t1.annee_reservation FROM reservations AS t1 WHERE (t1.id_client=:id_client AND (t1.date_don>='".($annee_premiere_adhesion+$i)."-10-01' AND t1.date_don<='".($annee_premiere_adhesion+$i+1)."-09-30') AND (t1.id_etat=1 OR t1.id_etat=2) ) GROUP BY t1.annee_reservation DESC");

				
				$sql_exec=$sql_dons->execute([":id_client"=>$row["id_client"]]);	
				
				if(!$sql_exec) echo "MODIFIER 2 - DONNEES : Pb d'accès à la table clients_dons <br />";
				else
				{	
			...
			else
			{
				...
				{					
					...
					// VERSION APRES CHANGEMENT DE l'ANNEE FISCALE PRISE EN COMPTE POUR LES DONS - DEBUT 2023-12-27 
					$sql_dons=$connexion->prepare("SELECT SUM(t1.don) as total_don, t1.id_client, t1.annee_reservation FROM reservations AS t1 WHERE (t1.id_client=:id_client AND (t1.date_don>=:debut_exercice_fiscal AND t1.date_don<=:fin_exercice_fiscal) AND (t1.id_etat=1 OR t1.id_etat=2) ) GROUP BY t1.annee_reservation ORDER BY t1.annee_reservation DESC");
...
{	
	...
	$sql_exec=$sql->execute([":association"=>$_POST["association"], ":nom"=>$_POST["nom"], ":prenom"=>$_POST["prenom"], ":adresse1"=>$_POST["adresse1"],  ":adresse2"=>$_POST["adresse2"],  ":adresse3"=>$_POST["adresse3"], ":cp"=>$_POST["cp"],  ":ville"=>$_POST["ville"], ":telephone"=>$_POST["telephone"], ":email"=>$_POST["email"], ":commentaire"=>$_POST["commentaire"], ":cle_client"=>uniqid(), ":date_creation"=>date("Y-m-d"),  ":date_modification"=>date("Y-m-d"), ":id_etat"=>'1', ":id_statut_client"=>$_POST["id_statut_client"], ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
	...
	else
	{
		...
		$sql_exec=$sql->execute([":id_client"=>$id_client_selectionne, ":annee"=>$_POST["annee_0"],  ":montant"=>$_POST["montant_0"], ":date_creation"=>date("Y-m-d"), ":date_modification"=>date("Y-m-d"), ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
...
elseif(isset($action_selectionne) AND $action_selectionne=="copier-valider" AND $_SESSION["droit"]==1) 
{
	...
	$sql_exec=$sql->execute([":association"=>$_POST["association"], ":nom"=>$_POST["nom"], ":prenom"=>$_POST["prenom"],  ":adresse1"=>$_POST["adresse1"],  ":adresse2"=>$_POST["adresse2"],  ":adresse3"=>$_POST["adresse3"], ":cp"=>$_POST["cp"],  ":ville"=>$_POST["ville"], ":telephone"=>$_POST["telephone"], ":email"=>$_POST["email"], ":commentaire"=>$_POST["commentaire"], ":date_creation"=>date("Y-m-d"),  ":date_modification"=>date("Y-m-d"), ":id_statut_client"=>$_POST["id_statut_client"], ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
...
elseif(isset($action_selectionne) AND $action_selectionne=="modifier-valider" AND $_SESSION["droit"]==1) 
{
	...
	$sql_exec=$sql->execute([":id_client"=>$_POST["id_client"], ":association"=>$_POST["association"], ":nom"=>$_POST["nom"], ":prenom"=>$_POST["prenom"], ":adresse1"=>$_POST["adresse1"], ":adresse2"=>$_POST["adresse2"], ":adresse3"=>$_POST["adresse3"], ":cp"=>$_POST["cp"], ":ville"=>$_POST["ville"], ":telephone"=>$_POST["telephone"], ":email"=>$_POST["email"], ":commentaire"=>$_POST["commentaire"], ":date_modification"=>date("Y-m-d"), ":id_statut_client"=>$_POST["id_statut_client"], ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
	...
	{		
		...
		{
			//print "test id_adhesion_0 trouvé<br />";
			...
			else	
			{
				...
				foreach ($sql_adhesions->fetchAll() as $row_adhesions) 
				{
					$compteur_0 ++;
					//print "test_0 A21 <br />";
					$sql_maj=$connexion->prepare("UPDATE clients_adhesions SET montant=:montant, date_modification=:date_modification, id_membre_auteur=:id_membre_auteur WHERE (id_adhesion=:id_adhesion)");
					$sql_exec=$sql_maj->execute([":id_adhesion"=>$_POST["id_adhesion_0"], ":montant"=>$_POST["montant_0"], ":date_modification"=>date("Y-m-d"), ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	

					if(!$sql_exec) echo "Modifier-Valider 3 : Pb d'accès à la table clients_adhesions";
				}
			}
			//print "compteur_0 =".$compteur_0."<br />";
			if($compteur_0==0)
			{
				//print "test_0 A31 <br />";
				...
				$sql_exec=$sql->execute([":id_client"=>$_POST["id_client"], ":annee"=>$_POST["annee_0"], ":montant"=>$_POST["montant_0"], ":date_creation"=>date("Y-m-d"), ":date_modification"=>date("Y-m-d"), ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
		...
		{
			//print "test id_adhesion_1 trouvé<br />";
			...
			else	
			{
				...
				foreach ($sql_adhesions->fetchAll() as $row_adhesions) 
				{
					...
					$compteur_1 ++;
					$sql_maj=$connexion->prepare("UPDATE clients_adhesions SET montant=:montant, date_modification=:date_modification, id_membre_auteur=:id_membre_auteur WHERE (id_adhesion=:id_adhesion)");
					$sql_exec=$sql_maj->execute([":id_adhesion"=>$_POST["id_adhesion_1"], ":montant"=>$_POST["montant_1"], ":date_modification"=>date("Y-m-d"), ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
			...
			if($compteur_1==0)
			{
				...
				$sql_exec=$sql->execute([":id_client"=>$_POST["id_client"], ":annee"=>$_POST["annee_1"], ":montant"=>$_POST["montant_1"], ":date_creation"=>date("Y-m-d"), ":date_modification"=>date("Y-m-d"), ":id_membre_auteur"=>$_SESSION["id_membre_auteur"]]);	
...
$liste_statuts = array();
$sql_statuts="SELECT * FROM clients_statuts ORDER BY libelle_statut ASC";
if(!$connexion->query($sql_statuts)) echo "Modifier : Pb d'accès à la table clients_statuts";
else
{
