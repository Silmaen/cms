...
$pdf->SetTitle('LISTE DES DONS PERCUS DANS UNE PERIODE');
$pdf->SetSubject('LISTE DES DONS PERCUS DANS UNE PERIODE');
$pdf->SetKeywords('LISTE DES DONS PERCUS DANS UNE PERIODE');
...
WHERE (t1.id_etat=1 AND t2.don>0 AND t2.date_don>=:date_debut_reglements_synthese AND t2.date_don<=:date_fin_reglements_synthese AND (t2.id_etat=1 OR t2.id_etat=2) ) 
ORDER BY t1.association ASC, t1.nom ASC, t1.prenom ASC, t2.date_don");
$sql_exec=$sql_dons->execute([":date_debut_reglements_synthese"=>$_POST["date_debut_reglements_synthese"], ":date_fin_reglements_synthese"=>$_POST["date_fin_reglements_synthese"]]);	
...
else
{
	...
	{							
		...
		{			
			...
			$pdf->MultiCell(200, 5, utf8_encode("<b>Détail des dons perçus entre le  ".$date_debut_reglements_synthese." et le ".$date_fin_reglements_synthese."</b>"), 0, 'C', 0, 1, 5, ($offset_titre_clients), true,'',true);
			
			$pdf->SetFont('Helvetica', '', 14);
			$pdf->MultiCell(200, 5, utf8_encode("<b>Liste des dons</b>"), 0, 'C', 0, 1, 5, ($offset_titre_clients+15), true,'',true);
			...
			$pdf->MultiCell(50, 7, utf8_encode("<b>Adhérent</b>"), 0, 'L', 0, 1, 65, ($offset_titre_colonnes), true,'',true);
			$pdf->MultiCell(25, 7, utf8_encode("<b>Départ le</b>"), 0, 'C', 0, 1, 115, ($offset_titre_colonnes), true,'',true);
...
$pdf->Output('fiche-dons-'.$_POST["date_debut_reglements_synthese"].'-'.$_POST["date_fin_reglements_synthese"].'.pdf', 'D');
