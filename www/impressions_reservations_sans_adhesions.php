...
else
{
	...
	foreach ($sql_clients->fetchAll() as $row_clients)
	{
		...
		if((fmod($compteur_client, $nb_clients_par_page)==0) AND ( $row_clients['premier_affichage']==1 OR ($row_clients['premier_affichage']==0) ) )
		{
			...
			$pdf->MultiCell(200, 5, utf8_encode("<b>Liste des adh�rents ayant fait au moins une r�servation sans avoir pay� d'adh�sion pour l'ann�e fiscale de r�f�rence : ".$annee_fiscale_reference_adhesion."-".($annee_fiscale_reference_adhesion+1)."</b>"), 0, 'C', 0, 1, 5, ($offset_titre_clients), true,'',true);
